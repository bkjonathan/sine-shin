  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      for (const [index, record] of records.entries()) {
        ...
        const rawPayment = getCsvField(record, ["payment method", "payment_method"]) ?? "";
        const paymentMethod =
        ...
        const expenseDate = (getCsvField(record, ["expense date", "expense_date"]) ?? "").trim() || undefined;
        const notes = (getCsvField(record, ["notes"]) ?? "").trim() || undefined;
        const importedId = (getCsvField(record, ["id", "uuid"]) ?? "").trim() || undefined;
        const expenseId = (getCsvField(record, ["expense id", "expense_id"]) ?? "").trim() || undefined;
        ...
        try {
          if (existing) {
            await updateExpense({
              ...
              payment_method: paymentMethod ?? existing.payment_method ?? undefined,
